...
static signed char gyro_orientation_original[9] = { 1, 0, 0,
                                                    0, 1, 0,
                                                    0, 0, 1};
...
// dmp��ʼ��, ����ֵ:0,����, ����,ʧ��
uint8_t mpu_dmp_init(uint8_t orient)
{
    uint8_t res = 0;
    if (mpu_init() == 0) //��ʼ��MPU
    {
        ...
        {
            res = dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation_original)); //���������Ƿ���
        }
        if (res) return 5;  
        //����dmp����
...
// �õ�dmp����������
uint8_t mpu_dmp_get_data(mpu_data_t *mpu_data)
{
    ...
    if (sensors & INV_WXYZ_QUAT)
    {
        ...
        //����õ�������/�����/�����
        mpu_data->roll = atan2(2*q2*q3+2*q0*q1, -2*q1*q1-2*q2*q2+1);     // roll
        mpu_data->pitch = asin(-2*q1*q3+2*q0*q2);                        // pitch
        mpu_data->yaw = atan2(2*(q1*q2+q0*q3), q0*q0+q1*q1-q2*q2-q3*q3); // yaw

        for (int i = 0; i < 3; i++)
        {
            mpu_data->accel[i] = accel[i];
            mpu_data->gyro[i] = gyro[i];
        }
    }
    else
        return 2;
