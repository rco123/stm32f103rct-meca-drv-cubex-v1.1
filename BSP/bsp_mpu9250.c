...
static uint8_t MPU_Basic_Init(void)
{
	...
	if (res == MPU6050_ID) // The device ID is correct //器件ID正确
	{
...
//初始化MPU9250
uint8_t MPU9250_Init(uint8_t enable_dmp)
{
	uint8_t res = 0;
	while (1)
	{
	...
	g_mpu_dmp = enable_dmp;
	while (enable_dmp)
	{
		int error = mpu_dmp_init(enable_dmp);        //初始化mpu_dmp库
		
		switch (error)                               //对不同错误值类型判断
		{
			case 0:printf("MPU DMP Init OK\r\n");break;
			case 1:printf("DMP sensor error\r\n");break;
			case 2:printf("DMP FIFO error\r\n");break;
			case 3:printf("DMP Samp rate error\r\n");break;
			case 4:printf("DMP Fireware error\r\n");break;
			case 5:printf("DMP gyro error\r\n");break;
			case 6:printf("DMP func error\r\n");break;
			case 7:printf("DMP out speed error\r\n");break;
			case 8:printf("DMP selftest error\r\n");break;
			case 9:printf("DMP enable error\r\n");break;
			case 10:printf("DMP init MPU9250 error\r\n");break;
			default :printf("DMP unknow error\r\n");break;
		}
...
#define LEN 23
	...
	data_buffer[3] = FUNC_REPORT_RAW_ICM; // 功能位
