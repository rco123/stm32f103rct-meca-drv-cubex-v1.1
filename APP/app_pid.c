...
// 设置PID参数
void PID_Set_Parm(pidx_t *pid, float p, float i, float d)
{
    pid->Kp = p; // 设置比例系数 P
    pid->Ki = i; // 设置积分系数 I
    pid->Kd = d; // 设置微分系数 D
...
// 设置PID的目标值
void PID_Set_Target(pidx_t *pid, float temp_val)
{
    pid->target_val = temp_val; // 设置当前的目标值
}

// 获取PID目标值
float PID_Get_Target(pidx_t *pid)
{
